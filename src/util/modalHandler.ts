...
export type ModalCompatibleInteraction = Exclude<Interaction, AutocompleteInteraction | ModalSubmitInteraction>;

export type ArrayOfOneToFiveItems<T> = [T] | [T, T] | [T, T, T] | [T, T, T, T] | [T, T, T, T, T];
...
export class Modal {
	 ...
	 */
	public static isValidModalData(data: ModalData): { valid: true; data: ModalData } | { valid: false; reason: string } {
		...
		const invalidPlaceholders = data.fields.some((field) => {
			...
			return placeholder === undefined ? false : !inRange(placeholder.length, 1, 100);
		});
	...
	private async show(
		...
		if (!validationResult.valid) return { success: false, reason: validationResult.reason };

		const modalBuilder = new ModalBuilder().setCustomId(this.data.customId).setTitle(this.data.title);
		...
		for (const field of this.data.fields) {
			const textInput = new TextInputBuilder()
				...
				.setStyle(field.style === "line" ? TextInputStyle.Short : TextInputStyle.Paragraph)
			...
			if (field.placeholder !== undefined) textInput.setPlaceholder(field.placeholder);
		...
		modalBuilder.addComponents(
			fields.map((f) => new ActionRowBuilder<ModalActionRowComponentBuilder>().addComponents(f)),
		);

		const result = await interaction.showModal(modalBuilder).catch((err: Error) => `Unhandled error occured: ${err}`);

		return typeof result === "string" ? { success: false, reason: result } : { success: true };
	}
		...
		return {
			...
			data: {
				...
				fieldValues: Array.from(
					submitResult.fields.fields.map((field) => ({ customId: field.customId, value: field.value })).values(),
