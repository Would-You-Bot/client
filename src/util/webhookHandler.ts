...
import { type IWebhookCache, WebhookCache } from "./Models/webhookCache";
import type WouldYou from "./wouldYou";

const cryptr = new Cryptr(process.env.ENCRYPTION_KEY as string);

export type WebHookCompatibleChannel = NewsChannel | StageChannel | TextChannel | VoiceChannel;
...
export default class WebhookHandler {
	...
	async handleWebhook(
		...
		if (message.webhook.id && message.webhook.token) {
			...
			try {
				const result = await this.send(webhookClient, content, overwriteProfile, message);
				if (result.success) {
				...
				if (newWebhook.success) {
					const result = await this.send(newWebhook.result, content, overwriteProfile, message);
					if (result.success) {
						if (!message.thread && !message.autoPin) return result;
						if (message.autoPin) {
							await this.autoPinMessage(message, result.result);
	...
	private async createWebhook(
		...
		if (!channel.permissionsFor(channel.guild.members.me)?.has([PermissionFlagsBits.ManageWebhooks]))
			return {
				success: false,
	...
	private async createThread(message: IQueueMessage, apiReturnValue: APIMessage): Promise<Result<string>> {
		const date = new Date();
		try {
	...
	private async autoPinMessage(message: IQueueMessage, apiReturnValue: APIMessage): Promise<Result<string>> {
		let pinChannel: any = await this.client.rest.get(`/channels/${message.channelId}/pins`);

		if (pinChannel) {
			pinChannel = pinChannel.filter((x: any) => x.application_id === this.client.user?.id);
		}
		...
		try {
			if (pinChannel && pinChannel.length > 0) {
				await this.client.rest.delete(`/channels/${pinChannel[0].channel_id}/pins/${pinChannel[0].id}`, {
					reason: "Automatic unpinning of daily message",
				});
			}

			this.client.rest.put(`/channels/${message.channelId}/pins/${apiReturnValue?.id}`, {
