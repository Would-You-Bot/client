...
export default class TranslationHandler {
	...
	get(language: string, translationPath: string, placeholders: Record<string, any> = {}): string {
		const languageData: Record<string, any> = this.getLanguage(language ?? "en_EN");
		...
		if (placeholders) {
			return currentTranslation.replace(/{(\w+)}/g, (match: string, key: string) => placeholders[key] ?? match);
		}
