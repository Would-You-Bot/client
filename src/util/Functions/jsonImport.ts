...
import { dareModel, nhieModel, topicModel, truthModel, wwydModel, wyrModel } from "../Models/questionModel";
...
type Quest = "truthQuestions" | "dareQuestions" | "wwydQuestions" | "nhieQuestions" | "wyrQuestions" | "topicQuestions";
...
export async function getQuestionsByType(
	...
	if (guildDb != null) {
		if (guildDb.customTypes !== "regular" && guildDb.customMessages.filter((e) => e.type === type).length === 0)
		...
		async function getDBQuestion(nin: string[]) {
			return await selectedModel.aggregate([{ $match: { id: { $nin: nin } } }, { $sample: { size: 1 } }]);
		}

		let questionDatabase = await getDBQuestion(premium && enabled ? usedQuestions[0][typeCheck[type]] : []);
		...
		if (!newRandomCustomQuestion[0]?.id && premium && enabled) {
			await reset(type as Quest, guildDb.customTypes, guildDb.guildID, "custom");
