...
export async function fileToCollection<Type extends Command | Interaction | Event>(
	...
	for (const dir of directories) {
		...
		const files = readdirSync(directoryPath).filter((file) => file.endsWith(".js"));

		for (const file of files) {
			const importPromise = import(path.join(directoryPath, file)).then((resp: { default: Type }) => {
				collection.set(
	...
	const rootFiles = dirents.filter((dirent) => !dirent.isDirectory() && dirent.name.endsWith(".js"));
	for (const file of rootFiles) {
		const importPromise = import(path.join(dirPath, file.name)).then((resp: { default: Type }) => {
			collection.set(
				(resp.default as Command).data !== undefined
					? (resp.default as Command).data.name
					: (resp.default as Event).event
