...
export default class DailyMessage {
	...
	private async sendDaily(message: IQueueMessage, properties: MessageProperties): Promise<Result<string>> {
		...
		const embed = this.buildEmbed(message.message[0], message.message[1], message.type, properties.messageId);
		if (!embed) {
			return {
				success: false,
				error: new Error(`Failed to build daily message embed for guild ${message.guildId}`),
			};
		}
		if (!channel.success) {
			return {
				success: false,
				error: new Error(`No channel has been fetched to post a daily message to! ${message.guildId}`, {
					cause: channel.error,
				}),
			};
		}
	 ...
	 * @author Nidrux
	 */
	private async getDailyMessageChannel(channelId: string): Promise<Result<WebHookCompatibleChannel>> {
		try {
			let channel = await this.client.channels.fetch(channelId);

			if (!channel) return { success: false, error: new Error("fetched channel is null") };
	...
	private async sendWebhook(
		...
		const premium: boolean = (await this.client.premium.check(channel.guildId)).result;
		...
		try {
			const result = await this.client.webhookHandler.handleWebhook(
				...
				channel,
				{
					...
					avatarURL: message.webhook.avatar || this.client.user?.displayAvatarURL({ forceStatic: false }),
