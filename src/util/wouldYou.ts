...
import { Client, Collection, GatewayIntentBits, LimitedCollection, Partials } from "discord.js";
	...
	constructor() {
		...
		this.cluster.on("ready", async () => {
			...
			this.database.connectToDatabase().then(() => {
				console.log(`${white("Would You?")} ${gray(">")} ${green("Successfully connected to the database")}`);
			});
