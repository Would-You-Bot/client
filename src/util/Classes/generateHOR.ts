...
export default class HOR {
	...
	async build(score: any) {
		...
		const [image1, image2] = await Promise.all([
			Canvas.loadImage(images[0]).catch((e) => Canvas.loadImage("https://cdn.wouldyoubot.gg/higherlower/error.png")),
			Canvas.loadImage(images[1]).catch((e) => Canvas.loadImage("https://cdn.wouldyoubot.gg/higherlower/error.png")),
		]);
		...
		ctx.textAlign = "center";
		ctx.fillText(`"${this.game.items.history[this.game.items.history.length - 1].keyword}"`, 356, 170);
		...
		ctx.textAlign = "center";
		ctx.fillText(`${formatNumber(this.game.items.history[this.game.items.history.length - 1].value)}`, 356, 285);
		...
		ctx.font = "50px Poppins-Bold";
		ctx.textAlign = "center";
		ctx.fillText(`"${this.game.items.current.keyword.slice(0, 17)}"`, 1068, 258);
