...
export default class LOSE {
	...
	async build(score: number, client: WouldYou) {
		...
		if (guild) {
			...
			} else if (score > guild?.gameScores.find((e: any) => e.userID === this.game.creator)?.higherlower!) {
				guild.gameScores = (guild.gameScores || []).filter(
					(gameScore: { userID: string }) => gameScore.userID !== this.game.creator,
				);
		...
		// Background
		const [image1] = await Promise.all([Canvas.loadImage("https://cdn.wouldyoubot.gg/higherlower/lose.png")]);
