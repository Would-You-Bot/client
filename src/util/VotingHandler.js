...
module.exports = class Voting {
  ...
  async saveVoting({ guildId, type = 0, until, channelId, op_one, op_tow }) {
    const randomId =
      ...
      case 1:
        row.addComponents([
          new ButtonBuilder()
            .setCustomId(`votingplaceholder`)
            .setLabel(
              this.client.translation.get(g?.language ?? "en_EN", "Voting.Vote")
            )
            .setDisabled(true)
            .setStyle(ButtonStyle.Secondary),
          new ButtonBuilder()
          ...
          new ButtonBuilder()
            ...
            .setLabel(
              op_tow ??
                this.client.translation.get(g?.language ?? "en_EN", "Voting.No")
            )
            .setStyle(ButtonStyle.Primary),
        ]);
        break;
  ...
  async updateImage(id) {
    ...
    const guildDb = await this.client.database.getGuild(vote.guildId, true);
    ...
    if (
      ...
      if (message && message.id) {
        await message.edit({
          files: [{ name: "wouldyoubot-either.png", attachment: image }],
        });
        return true;
      }
    }
  ...
  async getVotingResults(id) {
    ...
    const chartData = Object.keys(vote.options).map((e) =>
      Number(vote.options[`${e}`].length)
    );
  ...
  async endVoting(id, messageId) {
    ...
    if (vote.type !== 2) {
      ...
      message.components[0].components = message.components[0].components.slice(
        0,
        1
      );
      await message
  ...
  async runSchedule() {
    ...
    votes = [...votes].filter(
      (v) => v.until <= ~~(Date.now() / 1000) && v.until !== 0
    );
