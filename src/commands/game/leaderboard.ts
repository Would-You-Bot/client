...
const command: ChatInputCommand = {
	...
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option
				...
				.addChoices({ name: "Global", value: "global" }, { name: "Local", value: "local" })
				.setRequired(true),
		),
	...
	execute: async (interaction, client, guildDb) => {
		...
		const language = guildDb?.language != null ? guildDb.language : userDb.language ? userDb.language : "en_EN";
				...
				switch (interaction.options.getString("for")) {
					case "global": {
						...
						for (let i = 0; i < data; i++) {
							page.add(
								new EmbedBuilder().setTitle(client.translation.get(language, "Leaderboard.global")).setColor("#0598F6"),
							);
						}
						break;
					}
