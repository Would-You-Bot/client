	...
	execute: async (client: WouldYou, interaction: Interaction) => {
		...
		if (interaction.isCommand()) {
			...
			if (interaction.guildId !== null) {
				guildDb = await client.database.getGuild(interaction.guildId as string, true);
				client.database
			...
			if (!interaction.guild && !allowedInDMs.includes(interaction.commandName)) {
				interaction.reply({
					content:
			...
			await command
				.execute(interaction as ChatInputCommandInteraction<CacheType>, client, guildDb as IGuildModel)
				.catch((err) => {
