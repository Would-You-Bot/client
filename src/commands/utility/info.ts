...
const command: ChatInputCommand = {
	...
	execute: async (interaction, client, guildDb) => {
		...
		const userCount = await client.cluster.broadcastEval((c) => c.guilds.cache.reduce((a, b) => a + b.memberCount, 0));
		...
		const unixstamp = Math.floor(Date.now() / 1000) - Math.floor((client.uptime || 0) / 1000);
		...
		interaction.reply({ embeds: [infoEmbed], ephemeral: false }).catch((err) => {
