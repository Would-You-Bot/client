      ...
      subcommand
        ...
        .addNumberOption((option) =>
          option
            .setName("cooldown")
            .setDescription(
              "Change the cooldown for the replay button. Use seconds to determine how long."
            )
            .setRequired(true)
        )
    ),
  ...
  async execute(interaction, client, guildDb) {
    if (
      ...
      switch (interaction.options.getSubcommand()) {
        case "toggle":
          ...
          const doneEmbed = new EmbedBuilder()
            .setColor("#2f3037")
            .setTitle(
              client.translation.get(guildDb?.language, "REPLAY.embed.title")
            )
            .setDescription(
              !guildDb.replay
                ? `${client.translation.get(
                    guildDb?.language,
                    "REPLAY.embed.description"
        ...
        case "cooldown":
          ...
          const nochannelEmbed = new EmbedBuilder()
            ...
            .setDescription(
              `${client.translation.get(
                guildDb?.language,
                "REPLAY.embed.cooldownSuccess"
              )}\`${cooldown.toLocaleString()}\`${client.translation.get(
                guildDb?.language,
                "REPLAY.embed.cooldownSuccess2"
              )}`
            );
          await interaction
            .reply({
      ...
      const errorembed = new EmbedBuilder()
        ...
        .setTitle("Error!")
        .setDescription(
          client.translation.get(guildDb?.language, "REPLAY.embed.missingPerms")
        );
