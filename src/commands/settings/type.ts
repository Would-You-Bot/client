...
const command: ChatInputCommand = {
	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand.setName("regular").setDescription("This changes it to use only default messages"),
		)
		.addSubcommand((subcommand) =>
			subcommand.setName("mixed").setDescription("This changes it to use both custom & default messages"),
		)
		.addSubcommand((subcommand) =>
			subcommand.setName("custom").setDescription("This changes it to use only custom messages"),
		),
			...
			switch (interaction.options.getSubcommand()) {
				case "regular":
					...
					typeEmbed = new EmbedBuilder()
						.setTitle(client.translation.get(guildDb?.language, "wyType.embed.title"))
						.setDescription(client.translation.get(guildDb?.language, "wyType.embed.descDef"));
					break;
				case "mixed":
					await client.database.updateGuild(
						interaction.guildId || "",
					...
					typeEmbed = new EmbedBuilder()
						.setTitle(client.translation.get(guildDb?.language, "wyType.embed.title"))
						.setDescription(client.translation.get(guildDb?.language, "wyType.embed.descBoth"));
					break;
				case "custom":
					await client.database.updateGuild(
						interaction.guildId || "",
					...
					typeEmbed = new EmbedBuilder()
						.setTitle(client.translation.get(guildDb?.language, "wyType.embed.title"))
						.setDescription(client.translation.get(guildDb?.language, "wyType.embed.descCust"));
					break;
			}
