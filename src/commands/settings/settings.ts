...
const command: ChatInputCommand = {
	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((cmd) => {
			cmd.setName("cooldowns").setDescription("Change settings regarding cooldowns").setDescriptionLocalizations({
		...
		.addSubcommand((cmd) => {
			cmd.setName("qotd").setDescription("Change settings regarding the QOTD system").setDescriptionLocalizations({
		...
		.addSubcommand((cmd) => {
			cmd.setName("welcome").setDescription("Change settings regarding welcome messages").setDescriptionLocalizations({
		...
		.addSubcommand((cmd) => {
			cmd.setName("utility").setDescription("Change useful utility settings").setDescriptionLocalizations({
	...
	execute: async (interaction, client, guildDb) => {
		const memberHasPermissions = (interaction.member?.permissions as Readonly<PermissionsBitField>).has(
		...
		const cmd: SubCommandFunction = (await import(`./settings-subcommands/${subCommands[subCommand].fileName}.js`))
			.default;
