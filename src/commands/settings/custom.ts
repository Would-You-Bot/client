...
import { generateNHIE, generateWWYD, generateWYR } from "../../util/generateText";
		...
		if ((interaction?.member?.permissions as Readonly<PermissionsBitField>).has(PermissionFlagsBits.ManageGuild)) {
			switch (interaction.options.getSubcommand()) {
				case "add": {
					...
					if (!premium.result && guildDb.customMessages.filter((e) => e.type === option).length + 1 > 100) {
						const premiumButton = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
							new ButtonBuilder()
								.setLabel("Premium")
						...
						interaction.reply({
							content: `:x: ${client.translation.get(guildDb?.language, "wyCustom.error.limit", {
								type: `\`${option}\``,
							})}`,
					...
					const newID = uuidv4();
					if (option === "wouldyourather") generativeText = generateWYR(client, message || "", newID, guildDb);
					else if (option === "neverhaveiever") generativeText = generateNHIE(client, message || "", newID, guildDb);
					else if (option === "wwyd") generativeText = generateWWYD(client, message || "", newID, guildDb);
						...
						.setDescription(
							`${
								generativeText.value
									? ""
									: `${client.translation.get(guildDb?.language, "wyCustom.success.embedAdd.descAccept", {
											type:
					...
					const add = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
						new ButtonBuilder().setLabel("Add").setStyle(ButtonStyle.Primary).setCustomId(`wycustom_add-${newID}`),
						new ButtonBuilder()
							.setLabel("Don't Add")
					...
					const addDisable = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
						new ButtonBuilder()
							.setLabel("Add")
				...
				case "remove": {
					...
					const filtered = guildDb.customMessages.filter((c) => c.id !== message);
				...
				case "view": {
					...
					if (guildDb.customMessages.filter((c) => c.type === "neverhaveiever").length > 0) {
						let data: any;
					...
					if (guildDb.customMessages.filter((c) => c.type === "wouldyourather").length > 0) {
						let data: any;
						data = guildDb.customMessages
					...
					if (guildDb.customMessages.filter((c) => c.type === "truth").length > 0) {
						let data: any;
						data = guildDb.customMessages
					...
					if (guildDb.customMessages.filter((c) => c.type === "dare").length > 0) {
						let data: any;
						data = guildDb.customMessages
					...
					if (guildDb.customMessages.filter((c) => c.type === "wwyd").length > 0) {
						let data: any;
						data = guildDb.customMessages
					...
					if (guildDb.customMessages.filter((c) => c.type === "topic").length > 0) {
						let data: any;
						data = guildDb.customMessages.filter((c) => c.type === "topic").map((s, i) => `${s.id}: ${s.question}`);
						data = Array.from({ length: Math.ceil(data.length / 5) }, (a, r) => data.slice(r * 5, r * 5 + 5));
					...
					axios
						...
						.then(async (response) => {
							...
							for (const key in response.data) {
								...
								if (guildDb.customMessages.filter((e) => e.type === key).length + response.data[key].length > 100)
									all.push(key);
							}
							...
							for (const key in response.data) {
								...
								if (
									...
									guildDb.customMessages.filter((e) => e.type === key).length + response.data[key].length > 100
								) {
									const premiumButton = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
										new ButtonBuilder()
											.setLabel("Premium")
							...
							if (response.data.wouldyourather) {
								let i = guildDb.customMessages.filter((e) => e.type === "wouldyourather").length;
							...
							if (response.data.truth) {
								let i = guildDb.customMessages.filter((e) => e.type === "truth").length;
							...
							if (response.data.dare) {
								let i = guildDb.customMessages.filter((e) => e.type === "dare").length;
							...
							if (response.data.neverhaveiever) {
								let i = guildDb.customMessages.filter((e) => e.type === "neverhaveiever").length;
							...
							if (response.data.wwyd) {
								let i = guildDb.customMessages.filter((e) => e.type === "wwyd").length;
