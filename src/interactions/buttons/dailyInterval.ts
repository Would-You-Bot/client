  ...
  async execute(interaction: ButtonInteraction, client: ExtendedClient, guildDb: GuildProfileDocument) {
    ...
    interaction
      .awaitModalSubmit({
         ...
         * @param modalInt
         */
        filter: (modalInt: ModalSubmitInteraction) => modalInt.customId === modalObject.custom_id,
        time: 60000,
      })
      .then(async (modalInteraction: ModalSubmitInteraction) => {
        ...
        const useButtonStyle = guildDb.dailyMsg ? ButtonStyle.Success : ButtonStyle.Secondary;
