...
const button: Button = {
	...
	execute: async (interaction, client, guildDb) => {
		if (
			...
			interaction?.message.embeds[0]?.footer?.text.split(" | ")[0] !== interaction.user.tag
		) {
			interaction.reply({
				content: client.translation.get(guildDb?.language, "HigherLower.error.user", {
		...
		if (game && game?.items.current.value >= game?.items.history[game.items.history.length - 1].value) {
			game.score += 1;
			...
			const gameEmbed = new EmbedBuilder()
				...
				.setDescription(
					client.translation.get(guildDb?.language != null ? guildDb.language : "en_EN", "HigherLower.description", {
			...
			gameImage.build(game.score).then((image) => {
				const guessRow = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
					new ButtonBuilder().setCustomId(`higher_${game?.id}`).setLabel("Higher").setStyle(ButtonStyle.Success),
					new ButtonBuilder().setCustomId(`lower_${game?.id}`).setLabel("Lower").setStyle(ButtonStyle.Danger),
				);

				interaction.editReply({
					embeds: [gameEmbed],
					files: [new AttachmentBuilder(image).setFile(image).setName("game.png").setSpoiler(false)],
					components: [guessRow],
				});
			});
		} else {
			const loseEmbed = new EmbedBuilder()
				.setTitle(
					client.translation.get(guildDb?.language != null ? guildDb.language : "en_EN", "HigherLower.game.title"),
				)
				.setDescription(
