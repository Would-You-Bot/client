...
const button: Button = {
	...
	execute: async (interaction, client, guildDb) => {
		const arr =
			guildDb.replayChannels.filter((c) => c.id !== (interaction as any).values[0]).length > 0
		...
		if (guildDb.replayType === "Channels") {
			setDeleteButtons = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
				new ButtonBuilder()
