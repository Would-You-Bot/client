...
const buttonInteractionEvent: Event = {
	...
	execute: async (client: WouldYou, interaction: ButtonInteraction) => {
		...
		if (customId[3]) {
			if (customId[2] === "neverhaveiever" || customId[2] === "wouldyourather") {
				...
				await UserModel.updateOne({ userID: interaction.user?.id }, { $inc: { [fieldName]: 1 } });
			}
		}
		...
		if (
			...
			cooldown = Number(guildDb.replayChannels.find((x: any) => x.id === interaction.channelId)?.cooldown);
		...
		if (cooldownKey && cooldown!) {
			if (client.used.has(cooldownKey) && client.used.get(cooldownKey)! > Date.now()) {
				interaction
					.reply({
