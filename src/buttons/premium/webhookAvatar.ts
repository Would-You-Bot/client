...
const button: Button = {
	...
	execute: async (interaction, client, guildDb) => {
		...
		if (!regex.test(value!)) {
			data?.modal.reply({
				content: client.translation.get(guildDb?.language, "Settings.invalidLink"),
				ephemeral: true,
			});
			return;
		}
		...
		const button = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
			new ButtonBuilder()
				...
				.setStyle(guildDb.webhookName ? ButtonStyle.Success : ButtonStyle.Secondary),
			new ButtonBuilder()
		...
		const upload = await bucket.upload(image, `${guildDb.guildID}-${uuid()}`, undefined, "image");
