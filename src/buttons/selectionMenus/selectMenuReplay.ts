...
const button: Button = {
	...
	execute: async (interaction, client, guildDb) => {
		...
		if (Number(value) < 2) {
			data?.modal.reply({
				ephemeral: true,
				content: client.translation.get(guildDb?.language, "Settings.replayCooldownMin"),
			});
			return;
		}
		...
		if (guildDb.replayChannels.find((e: any) => e.id === (interaction as any).values[0])) {
			guildDb.replayChannels = guildDb.replayChannels.filter((c) => c.id !== (interaction as any).values[0]);
		}
			...
			.setDescription(
				`${client.translation.get(
					...
					guildDb.replayBy === "Guild"
						? client.translation.get(guildDb?.language, "Settings.embed.replayBy2")
						: client.translation.get(guildDb?.language, "Settings.embed.replayBy1")
		...
		const generalButtons = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
			...
			new ButtonBuilder()
				...
				.setLabel(client.translation.get(guildDb?.language, "Settings.button.replayBy"))
				.setStyle(ButtonStyle.Primary)
				.setEmoji("1207778786976989244"),
		);

		const setDeleteButtons = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
			new ButtonBuilder()
