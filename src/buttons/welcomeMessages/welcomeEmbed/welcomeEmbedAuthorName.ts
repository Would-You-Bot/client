...
const button: Button = {
  name: "welcomeEmbedAuthorName",
  cooldown: false,
  execute: async (interaction, client, guildDb) => {
    const { data } = await new Modal({
      title: "Set Embed Author Name",
      customId: "WelcomeEmbedEdit",
      fields: [
        {
          customId: "input",
          style: "line",
          label: "What should the author name be?",
          required: true,
    ...
    const schema = welcomeEmbed({ guildDb: guildDb, author: value });
    ...
    const welcome = embed({ client: client, guildDb: guildDb, author: value });
    const welcomeButtons = Button1({
      client: client,
      guildDb: guildDb,
      author: ButtonStyle.Success,
    });
