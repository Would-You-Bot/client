...
const button: Button = {
	...
	execute: async (interaction, client, guildDb) => {
		...
		const button = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
			new ButtonBuilder().setLabel("Add").setStyle(ButtonStyle.Primary).setDisabled(true).setCustomId("add"),
			new ButtonBuilder().setLabel("Don't Add").setDisabled(true).setStyle(ButtonStyle.Secondary).setCustomId("remove"),
		);
