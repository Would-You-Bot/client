...
const event: Event = {
	...
	execute: async (client: WouldYou, id: number) => {
		console.log(`${white("Would You?")} ${gray(">")} ${green(`Shard is now ready #${id}`)}`);
		...
		const randomStatus = client.config.status[Math.floor(Math.random() * client.config.status.length)];
