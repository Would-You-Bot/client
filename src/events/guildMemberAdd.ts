...
import { type GuildMember, type GuildTextBasedChannel, PermissionFlagsBits } from "discord.js";
...
const event: Event = {
	...
	execute: async (client: WouldYou, member: GuildMember) => {
		...
		const channel = (await member.guild.channels.fetch(guildDb.welcomeChannel).catch((err: Error) => {
