...
const key = crypto.createHash('sha512').update(config.env.SECRET_KEY).digest('hex').substring(0, 32);
const encryptionIV = crypto.createHash('sha512').update(config.env.SECRET_IV).digest('hex').substring(0, 16);
